...
def _get_product_from_key(s3_key: str) -> str | None:
    """Extract the AHI product code (CMSK, CHGT, CPHS) from an S3 key."""
    for product in ("CMSK", "CHGT", "CPHS"):
        if f"AHI-{product}_" in s3_key:
            return product
    return None
...
def generate_reference_from_template(
    ...
    template_path:
        Local path to a template reference JSON.  If produced by
        ``scripts/create_product_templates.py`` the JSON contains a
        ``_template_source`` field that is used automatically when
        *template_s3_key* is ``None``.
    cache_dir:
        Root directory for caching.
    template_s3_key:
        The S3 key whose URL appears in the template and must be replaced.
        If ``None``, the key is read from ``_template_source`` inside the
        template JSON.
    force:
    ...
    if template_s3_key is None:
        source = json.loads(content).get("_template_source")
        if not source:
            raise ValueError(
                "template_s3_key not provided and '_template_source' not found in "
                f"{template_path}.  Use a template generated by "
                "scripts/create_product_templates.py, or pass template_s3_key explicitly."
            )
        template_s3_key = source
...
def build_references_parallel(
    ...
    **templates_dir** (fastest, safest)
        Directory produced by ``scripts/create_product_templates.py``.  The
        product code is inferred from the S3 key and the matching
        ``{PRODUCT}_template.json`` is used to derive all uncached references
        via URL substitution — **zero S3 round-trips**.

    **use_template=True** (fast, opt-in)
        The first available cached reference is used as a dynamic template for
        the remaining files.  Requires all files to share the same internal HDF5
        chunk byte offsets.  Disabled by default; only enable after verifying
        the assumption holds for the target product.

    **default** (safe, slow)
        Each file's HDF5 metadata is parsed independently from S3.
    ...
    use_template:
        Dynamic template from the first available reference.  Ignored when
        *templates_dir* is provided.
    templates_dir:
        Directory containing ``{PRODUCT}_template.json`` files produced by
    ...
    # ── Mode 1: explicit product template ────────────────────────────────────
    if templates_dir is not None:
        product = _get_product_from_key(uncached[0])
        explicit_template: Path | None = None
        if product is not None:
            candidate = Path(templates_dir) / f"{product}_template.json"
            if candidate.exists():
                explicit_template = candidate
                logger.info("Using product template: %s", candidate.name)
            else:
                logger.warning(
                    "Template %s not found; falling back to S3 generation.", candidate
                )
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as pool:
                futures = {
                    pool.submit(
                        ...
                        key, explicit_template, cache_dir, None, force,
                    ): key
                    for key in uncached
                }
    ...
    if use_template:
        ...
        if uncached:
            dyn_template = _ref_cache_path(template_key, cache_dir)
            logger.info(
                "Building %d references from dynamic template %s",
                len(uncached),
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as pool:
                futures = {
                    pool.submit(
                        generate_reference_from_template,
                        key, dyn_template, cache_dir, template_key, force,
                    ): key
                    for key in uncached
                }
                for future in as_completed(futures):
    ...
    # ── Mode 3: independent S3 generation (default) ──────────────────────────
    else:
        fs = _make_fs()
        with ThreadPoolExecutor(max_workers=max_workers) as pool:
            futures = {
...
def load_gbr_cloud_data(
    ...
    templates_dir:
        Directory containing ``{PRODUCT}_template.json`` files produced by
        ``scripts/create_product_templates.py``.  When provided, per-file
        references are derived via URL substitution instead of S3 HDF5 parsing.
